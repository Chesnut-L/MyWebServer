...
public:
    ...
    static const int WRITE_BUFFER_SIZE = 2048;      // 写缓冲大小
...
public:
    ...
    void init(int sockfd, const sockaddr_in & sockaddr);    // 初始化新接收的连接
...
private:
    ...
    char * m_version;   // 协议版本，只支持HTTP1.0
    METHOD m_method;    // 请求方法
    char * m_host;      // 主机名
    bool m_linger;      // 判断HTTP是否需要保持连接
    int m_content_len;  // 请求体内容长度 
    ...
    int m_write_idx;                        // 标识写缓冲区中已经读入的客户端数据的最后一个字节的下一个位置
